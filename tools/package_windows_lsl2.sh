...
OUT="$DIST/LSL2-CHT-win64-full.zip"
...
docker run --rm --name lsl2-winpkg-strip -v "$STAGE:/s" "$MINGW_IMG" x86_64-w64-mingw32-strip /s/scummvm.exe

echo ">> 收集 mingw runtime DLL（objdump 確認只需 SDL2.dll + libwinpthread-1.dll，其餘皆系統內建）"
docker run --rm --name lsl2-winpkg-sdl2dll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/bin/SDL2.dll > "$STAGE/SDL2.dll"
docker run --rm --name lsl2-winpkg-pthreaddll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/lib/libwinpthread-1.dll > "$STAGE/libwinpthread-1.dll"
...
幻想空間II（Leisure Suit Larry 2: Goes Looking for Love）繁體中文化 — Windows x86_64 完整包

雙擊「玩-幻想空間2-繁中.bat」即可開始遊戲。
...
repo（patch-only，不含遊戲資源/ROM）：https://github.com/wicanr2/leisure_suit_larry2-goes-lookling-for-love-ega-cht
TXT
