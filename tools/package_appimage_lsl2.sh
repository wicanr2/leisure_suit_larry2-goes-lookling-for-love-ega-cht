...
APPDIR="$STAGE/AppDir"
OUT="$DIST/LSL2-CHT-full-x86_64.AppImage"
...
docker run --rm --name lsl2-pkg-strip -v "$APPDIR/usr/bin:/b" qfg1-build:latest strip /b/scummvm 2>/dev/null || true

echo ">> 收集共享庫(qfg1-build 內 ldd,排除 glibc 核心)"
docker run --rm --name lsl2-pkg-libs \
...
Type=Application
Name=幻想空間II（繁體中文版）
Comment=Leisure Suit Larry 2: Goes Looking for Love 繁體中文化 — ScummVM patch
Exec=AppRun
Icon=lsl2-cht
Categories=Game;
Terminal=false
...
docker run --rm --name lsl2-pkg-appimagetool -v "$STAGE:/stage" -v "$ROOT/tools/.cache:/cache:ro" -e ARCH=x86_64 -w /stage \
